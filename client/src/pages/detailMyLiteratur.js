...
import DetailMyLiteratur from "../commponent/my-profile/dtlLiterature";
import { API } from "../config/api";
import { useParams } from "react-router";

function DetailMyLiteratures() {
  const title = "Detail";
          ...
          <Container>
            ...
            <div className="d-flex flex-wrap justify-content-start">
              {literatur.map((item, index) => (
                <DetailMyLiteratur item={item} key={index} />
              ))}
            </div>
          </Container>
        </div>
...
export default DetailMyLiteratures;
